...
- 验证跨模块同步，例如任务、日程、学习计划、收支和笔记变化后仪表盘摘要与 AI 本地上下文同步更新，以及控制台生成的 AI 草稿能被列表、查看、确认和取消入口共享访问。
