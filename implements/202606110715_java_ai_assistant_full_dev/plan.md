...
## R21 PASSED 实现 AI 结构化建议草稿模型、解析器与仓储基础
结果：新增 `assistant.ai.SuggestionDraftType`、`SuggestionDraftStatus`、`TaskDraftItem`、`StudyPlanDraftContent`、`SuggestionDraft`、`SuggestionDraftView`、`SuggestionDraftRepository`、`InMemorySuggestionDraftRepository`、`StructuredSuggestionParser`，实现结构化建议草稿类型、状态流转、任务/学习计划草稿内容、只读快照、内存持有和严格 JSON/fenced JSON 解析。
测试：`mvn test` 通过；验证报告记录通过 793 个测试，失败 0 个，推送成功。

## R21 NEW 实现 AI 草稿生命周期与正式导入服务闭环
任务：新增 AI 草稿生命周期服务和草稿导入服务，预期文件路径包括 `java-ai-assistant/src/main/java/assistant/ai/DraftImportService.java`、`java-ai-assistant/src/main/java/assistant/ai/DraftLifecycleService.java`、`java-ai-assistant/src/test/java/assistant/ai/DraftImportServiceTest.java`、`java-ai-assistant/src/test/java/assistant/ai/DraftLifecycleServiceTest.java`。
选择理由：v20 已完成 AI 结构化建议草稿的受控中间表示、解析器和仓储；技术方案中剩余 AI 结构化建议主线是“用户确认或取消后导入正式记录”。先在 `assistant.ai` 中补齐生命周期与导入服务，可让后续控制台菜单只调用服务接口，不把状态检查、重复确认、防半写入和回滚逻辑散落到交互层。
上下文：既有 `SuggestionDraftRepository` 持有 `SuggestionDraft` 聚合根，`SuggestionDraft.cancel()` 和 `markImported()` 已保护 `CONFIRMABLE -> CANCELLED/IMPORTED` 状态迁移并在终态重复迁移时抛出 `BusinessException(ErrorCode.STATE_CONFLICT, ...)`；既有 `TaskService.createTask(...)`、`TaskService.deleteTask(...)`、`StudyPlanService.createStudyPlan(...)` 可作为正式数据写入入口，导入不得直接操作任务或学习计划仓储。技术方案要求确认不存在草稿返回 `NOT_FOUND`，取消不存在草稿返回 `NOT_FOUND`，重复确认、取消后确认、已导入后确认或终态取消返回 `STATE_CONFLICT` 且不得新增正式数据；任务草稿批量导入必须保证正式数据不半写入，若任一任务创建失败或后续异常，应删除本次已创建任务并保持草稿 `CONFIRMABLE`；学习计划草稿导入失败也应保持草稿 `CONFIRMABLE`。导入成功后草稿状态必须保存为 `IMPORTED` 并返回导入后的草稿只读视图；取消成功后保存 `CANCELLED` 视图；列表和查看接口返回 `SuggestionDraftView` 或不可修改的 `List<SuggestionDraftView>`，不得暴露内部可变草稿。普通单元测试不得访问真实网络、真实 DeepSeek、真实 API Key、真实当前时间或外部文件。
...
## R26 PASSED 实现控制台学习计划完整交互入口
结果：扩展 `assistant.app.ConsoleApplication` 的学习计划入口，将主菜单命令 `4` 接入可循环学习计划子菜单，实现学习计划新增、查看、列表、筛选、修改详情、更新进度、删除、输入校验、帮助、返回主菜单和 EOF 处理；同步补充控制台交互测试。
测试：`mvn test` 通过；验证报告记录通过 896 个测试，失败 0 个。

## R26 NEW 实现控制台收支记录完整交互入口
