...
| 测试运行 | 使用 Maven Surefire 3.6.0 运行单元测试；集成测试使用 Maven profile 单独开启，避免默认测试依赖网络和 API Key。 |
...
`pom.xml` 固定 Java release 为 17。依赖只放置 Jackson、JUnit Jupiter、Mockito，Maven 插件配置 Surefire、Failsafe、JaCoCo 和可选的 Exec 插件。普通命令为：
